...
function mutationPath(name, args) {
    if (!MUTATION_TOOLS.has(name) || typeof args !== 'object' || args === null || Array.isArray(args))
        return undefined;
    const record = args;
    if (name === 'str_replace_editor') {
        if (!new Set(['create', 'str_replace', 'insert']).has(String(record.command)))
            return undefined;
...
export function detectStoryMutation(name, args, cwd) {
    const rawPath = mutationPath(name, args);
    if (cwd === undefined || rawPath === undefined)
        return undefined;
...
async function storyMutation(exec, fs) {
    ...
    try {
        ...
        if (!fs.contains(rootTarget, mutationTarget))
            return undefined;
    }
    catch {
        return undefined;
...
export async function decideStoryMutation(exec, next) {
...
//# sourceMappingURL=native-hooks.js.map