...
export const PRODUCTION_INTENT_SECTIONS = ['shots', 'assets', 'tasks', 'sequence', 'canvas'];
export const PRODUCTION_INTENT_JOB_KINDS = ['image', 'video', 'composition'];
function requiredText(value, field) {
    const normalized = value?.trim();
    if (!normalized)
        throw new Error(`oh_story_production ${field} is required for this action.`);
    if (normalized.length > 512)
        throw new Error(`oh_story_production ${field} is too long.`);
    return normalized;
}
/** Validate the cross-runtime UI intent without reading or mutating workspace state. */
export function validateProductionIntent(args) {
    ...
    if (args.action === 'open_section') {
        if (args.section === undefined)
            throw new Error('oh_story_production section is required for open_section.');
    ...
    if (args.action === 'set_sequence') {
        ...
        if (shotIds.length === 0)
            throw new Error('oh_story_production shotIds must contain at least one shot for set_sequence.');
    ...
    if (args.jobKind === undefined)
        throw new Error('oh_story_production jobKind is required for track_job.');
...
//# sourceMappingURL=production-intent.js.map