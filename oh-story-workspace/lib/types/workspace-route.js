...
const STORY_DIRECTORIES = ['正文', '大纲', '设定', '追踪', '对标', '参考资料'];
const DRAMA_DIRECTORIES = ['输入', '项目开发', '设定集', '剧集', '交付', '创作者决策', '审查'];
...
async function jsonBody(request, maxBytes) {
    ...
    for await (const chunk of request) {
        ...
        if (size > maxBytes)
            throw new WorkspaceHttpError(413, '请求内容过大。');
        chunks.push(value);
    }
    try {
...
export function assertCreativePath(path, kind) {
...
async function workspaceRealmForSession(context, rawId) {
    ...
    if (cwd === undefined)
        throw new WorkspaceHttpError(409, '当前 DSH 会话没有工作目录。');
    const fs = agent.ctx.get('fs');
    const sandboxPolicy = agent.ctx.get('sandboxPolicy');
    if (fs === undefined || sandboxPolicy === undefined)
        throw new WorkspaceHttpError(503, 'DSH 文件系统当前不可用。');
...
async function creativeTarget(realm, path, kind = 'text') {
    ...
    if (!realm.fs.contains(realm.root, target))
        throw new WorkspaceHttpError(403, '文件路径离开了 DSH 工作目录。');
    return target;
}
function requireRegularFile(info) {
    if (info === undefined)
        throw new WorkspaceHttpError(404, '文件不存在。');
    if (info.type !== 'file')
        throw new WorkspaceHttpError(415, '目标不是可编辑的普通文件。');
    return info;
}
...
async function readVersionedFile(fs, target, maxBytes) {
    for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        if (before.size !== undefined && before.size > maxBytes)
            throw new WorkspaceHttpError(413, '文件超过工作台大小限制。');
...
async function listFiles(realm) {
    ...
    const walk = async (path, directory) => {
        for (const entry of await realm.fs.listDir(directory)) {
            if (entry.name.startsWith('.') || !realm.fs.contains(realm.root, entry.target))
                continue;
            const childPath = `${path}/${entry.name}`;
            if (entry.type === 'directory')
                await walk(childPath, entry.target);
...
async function handle(context, request, response, options) {
    try {
        ...
        if (url.pathname === '/oh-story/workspace' && request.method === 'GET') {
            ...
            const metadataErrors = [tracking.error, shortDrama.error].filter((value) => value !== undefined);
        ...
        if (url.pathname === '/oh-story/file' && request.method === 'GET') {
            ...
            if (path === null)
                throw new WorkspaceHttpError(400, '缺少文件路径。');
        ...
        if (url.pathname === '/oh-story/media' && request.method === 'GET') {
            ...
            const path = url.searchParams.get('path');
            if (path === null)
                throw new WorkspaceHttpError(400, '缺少媒体文件路径。');
            const mimeType = mediaMimeTypeForPath(path);
            if (mimeType === undefined)
                throw new WorkspaceHttpError(415, '目标不是受支持的短剧媒体文件。');
            const target = await creativeTarget(realm, path, 'media');
            const info = requireRegularFile(await realm.fs.stat(target));
            if (info.size !== undefined && info.size > MEDIA_MAX_BYTES)
                throw new WorkspaceHttpError(413, '媒体文件超过工作台预览大小限制。');
        ...
        if (url.pathname === '/oh-story/file' && request.method === 'PUT') {
            ...
            if (typeof input.content !== 'string')
                throw new WorkspaceHttpError(400, 'content 必须是字符串。');
            if (typeof input.baseVersion !== 'string' || input.baseVersion === '')
                throw new WorkspaceHttpError(400, 'baseVersion 必须是有效版本。');
            if (Buffer.byteLength(input.content) > options.maxBytes)
                throw new WorkspaceHttpError(413, '文件超过工作台大小限制。');
            const outcome = await realm.fs.writeText(await creativeTarget(realm, path), input.content, { kind: 'replaceIfVersion', version: input.baseVersion }, undefined, realm.sandboxPolicy.resolve({ session: realm.agent.session }));
        ...
        if (mapped === undefined)
            context.logger('oh-story').error('workspace route failed', error);
...
export function registerWorkspaceRoute(context, options) {
