...
function settledMutationSignals(block: ToolCallBlock): string[] {
  ...
  const rawDiffs = typeof block.meta === 'object' && block.meta !== null && !Array.isArray(block.meta)
    ? (block.meta as Record<string, unknown>).diffs
    : undefined
  const diffs = Array.isArray(rawDiffs) ? rawDiffs.filter((value): value is {
    readonly path: string
    readonly oldText: string | null
    readonly newText: string
  } => typeof value === 'object' && value !== null
    && typeof (value as Record<string, unknown>).path === 'string'
    && (typeof (value as Record<string, unknown>).oldText === 'string' || (value as Record<string, unknown>).oldText === null)
    && typeof (value as Record<string, unknown>).newText === 'string') : undefined
