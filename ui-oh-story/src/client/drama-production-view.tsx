...
function ProductionCanvas(props: Props) {
  ...
  const shotChain = props.production.shots.slice(0, -1).map((shot, index) => [shot.id, props.production.shots[index + 1]?.id ?? shot.id] as const)
  ...
  return <section className="oh-story-canvas-shell" aria-label="短剧素材与镜头关系画布"><div className="oh-story-projection-note">文档关系 · 布局仅保存在当前 DSH Session</div><div className="oh-story-canvas-controls"><button type="button" aria-label="缩小画布" onClick={() => { props.onZoomChange(Math.max(.5, props.zoom - .1)) }}>−</button><span>{Math.round(props.zoom * 100)}%</span><button type="button" aria-label="放大画布" onClick={() => { props.onZoomChange(Math.min(1.8, props.zoom + .1)) }}>＋</button><button type="button" onClick={() => { props.onCanvasChange({}); props.onZoomChange(.65) }}>复位</button></div><div className="oh-story-canvas-legend" aria-label="连线图例"><span data-kind="reference">素材→镜头 {String(connections.length)} 条</span><span data-kind="sequence">镜头顺序 {String(shotChain.length)} 条</span></div><div className="oh-story-canvas-viewport"><div className="oh-story-canvas" style={{ transform: `scale(${String(props.zoom)})` }}><svg aria-hidden="true">{connections.map(([from, to]) => { const a = positions[from]; const b = positions[to]; if (a === undefined || b === undefined) return null; return <path key={`ref:${from}:${to}`} data-kind="reference" data-active={to === props.selectedId || undefined} d={`M ${String(a.x + 180)} ${String(a.y + 50)} C ${String(a.x + 360)} ${String(a.y + 50)}, ${String(b.x - 180)} ${String(b.y + 50)}, ${String(b.x)} ${String(b.y + 50)}`} /> })}{shotChain.map(([from, to]) => { const a = positions[from]; const b = positions[to]; if (a === undefined || b === undefined) return null; return <path key={`seq:${from}:${to}`} data-kind="sequence" data-active={to === props.selectedId || from === props.selectedId || undefined} d={`M ${String(a.x + 180)} ${String(a.y + 76)} C ${String(a.x + 180)} ${String(a.y + 116)}, ${String(b.x + 180)} ${String(b.y - 40)}, ${String(b.x + 180)} ${String(b.y)}`} /> })}</svg>{nodes.map(node => <article key={node.id} tabIndex={0} aria-label={`${node.type === 'asset' ? '素材' : '镜头'} ${node.label}`} data-node-type={node.type} data-selected={node.id === props.selectedId || undefined} style={{ left: positions[node.id]?.x, top: positions[node.id]?.y }} onKeyDown={(event) => { const step = event.shiftKey ? 40 : 10; const delta: readonly [number, number] | undefined = event.key === 'ArrowLeft' ? [-step, 0] : event.key === 'ArrowRight' ? [step, 0] : event.key === 'ArrowUp' ? [0, -step] : event.key === 'ArrowDown' ? [0, step] : undefined; if (delta !== undefined) { event.preventDefault(); moveNode(node.id, delta[0], delta[1]) } }} onPointerDown={(event) => { startDrag(event, node.id) }} onDoubleClick={() => { const target = props.production.targets.get(node.id); if (target !== undefined) props.onNavigate(target) }}><small>{node.type === 'asset' ? '素材' : '镜头'}</small><strong>{node.label}</strong><span>{node.id}</span></article>)}</div></div></section>
}
