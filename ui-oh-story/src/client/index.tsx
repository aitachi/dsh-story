...
import type { PartialAssistant, RunningToolCall } from '@deepseek-ai/dsh-client-ui-chat/client'
...
export function apply(context: ClientContext): void {
  const slots = context.get('slots') as unknown as SlotRegistry | undefined
  if (slots === undefined) throw new Error('oh-story: slots service unavailable')
  slots.inject('shell.overlay', () => {
    ...
    const disposeWorkbench = slots.register({
      ...
      inject: (sessionId) => {
        const session = (context.get('sessions') as unknown as ISessions | undefined)?.scope(sessionId)
