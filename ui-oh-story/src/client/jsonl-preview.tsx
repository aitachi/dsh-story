...
export function JsonlPreview({ content, label }: { readonly content: string; readonly label: string }) {
  ...
  if (records.length === 0) return <div className="oh-story-markdown-empty">这个 JSONL 文件还是空的。</div>
  ...
  return <section className="oh-story-jsonl" aria-label={`${label} 结构化预览`}>
    <header className="oh-story-jsonl-summary">
    ...
    <div className="oh-story-jsonl-records">
      {visible.map(record => record.error === undefined
        ...
        : <div className="oh-story-jsonl-error" key={record.line} role="alert">
          <strong>{record.title}</strong>
