...
function ProductionCanvas(props) {
    ...
    return _jsxs("section", { className: "oh-story-canvas-shell", "aria-label": "\u77ED\u5267\u7D20\u6750\u4E0E\u955C\u5934\u5173\u7CFB\u753B\u5E03", children: [_jsx("div", { className: "oh-story-projection-note", children: "\u6587\u6863\u5173\u7CFB \u00B7 \u5E03\u5C40\u4EC5\u4FDD\u5B58\u5728\u5F53\u524D DSH Session" }), _jsxs("div", { className: "oh-story-canvas-controls", children: [_jsx("button", { type: "button", "aria-label": "\u7F29\u5C0F\u753B\u5E03", onClick: () => { props.onZoomChange(Math.max(.5, props.zoom - .1)); }, children: "\u2212" }), _jsxs("span", { children: [Math.round(props.zoom * 100), "%"] }), _jsx("button", { type: "button", "aria-label": "\u653E\u5927\u753B\u5E03", onClick: () => { props.onZoomChange(Math.min(1.8, props.zoom + .1)); }, children: "\uFF0B" }), _jsx("button", { type: "button", onClick: () => { props.onCanvasChange({}); props.onZoomChange(.65); }, children: "\u590D\u4F4D" })] }), _jsxs("div", { className: "oh-story-canvas-legend", "aria-label": "\u8FDE\u7EBF\u56FE\u4F8B", children: [_jsxs("span", { "data-kind": "reference", children: ["\u7D20\u6750\u2192\u955C\u5934 ", String(connections.length), " \u6761"] }), _jsxs("span", { "data-kind": "sequence", children: ["\u955C\u5934\u987A\u5E8F ", String(shotChain.length), " \u6761"] })] }), _jsx("div", { className: "oh-story-canvas-viewport", children: _jsxs("div", { className: "oh-story-canvas", style: { transform: `scale(${String(props.zoom)})` }, children: [_jsxs("svg", { "aria-hidden": "true", children: [connections.map(([from, to]) => { const a = positions[from]; const b = positions[to]; if (a === undefined || b === undefined)
                                    return null; return _jsx("path", { "data-kind": "reference", "data-active": to === props.selectedId || undefined, d: `M ${String(a.x + 180)} ${String(a.y + 50)} C ${String(a.x + 360)} ${String(a.y + 50)}, ${String(b.x - 180)} ${String(b.y + 50)}, ${String(b.x)} ${String(b.y + 50)}` }, `ref:${from}:${to}`); }), shotChain.map(([from, to]) => { const a = positions[from]; const b = positions[to]; if (a === undefined || b === undefined)
                                    return null; return _jsx("path", { "data-kind": "sequence", "data-active": to === props.selectedId || from === props.selectedId || undefined, d: `M ${String(a.x + 180)} ${String(a.y + 76)} C ${String(a.x + 180)} ${String(a.y + 116)}, ${String(b.x + 180)} ${String(b.y - 40)}, ${String(b.x + 180)} ${String(b.y)}` }, `seq:${from}:${to}`); })] }), nodes.map(node => _jsxs("article", { tabIndex: 0, "aria-label": `${node.type === 'asset' ? '素材' : '镜头'} ${node.label}`, "data-node-type": node.type, "data-selected": node.id === props.selectedId || undefined, style: { left: positions[node.id]?.x, top: positions[node.id]?.y }, onKeyDown: (event) => { const step = event.shiftKey ? 40 : 10; const delta = event.key === 'ArrowLeft' ? [-step, 0] : event.key === 'ArrowRight' ? [step, 0] : event.key === 'ArrowUp' ? [0, -step] : event.key === 'ArrowDown' ? [0, step] : undefined; if (delta !== undefined) {
                                event.preventDefault();
