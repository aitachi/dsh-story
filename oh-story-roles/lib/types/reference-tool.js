...
async function collectMarkdown(root, directory = root) {
    ...
    const nested = await Promise.all(entries.map(async (entry) => {
        ...
        if (entry.isDirectory())
            return collectMarkdown(root, path);
...
export async function createOhStoryReferenceTool(storySetupRoot = join(defaultBundledSkillRoot(), 'story-setup')) {
    const references = await bundledReferences(storySetupRoot);
    if (references.length === 0)
        throw new Error('No bundled Oh Story references were found.');
    ...
    return defineTool({
        ...
        async execute(args) {
            ...
            if (path === undefined)
                throw new Error(`Oh Story reference is not bundled: ${args.reference}`);
...
export function bundledReferenceGuard(definition, tools) {
...
//# sourceMappingURL=reference-tool.js.map