...
import { bundledReferenceGuard, createOhStoryReferenceTool, OH_STORY_REFERENCE_TOOL_NAME, } from './reference-tool.js';
import { OH_STORY_ROLE_NAMES, loadBundledRole } from './role-provider.js';
...
export async function createOhStoryRoleTool(subagents) {
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            if (exec.agent === undefined)
                throw new Error('oh_story_role requires a calling DSH Agent.');
            const persona = personas.get(args.role);
            if (persona === undefined)
                throw new Error(`Oh Story Role ${args.role} is not bundled.`);
            const allowed = roleToolFilter(args.role).allow.filter(name => exec.agent?.ctx.tools.get(name, exec.agent) !== undefined);
            const runtime = subagents ?? exec.agent.ctx.get('subagents');
            if (runtime === undefined)
                throw new Error('oh_story_role requires the DSH subagent runtime.');
...
export async function registerOhStoryRoleTool(context) {
    ...
    let dispose;
    const mount = () => { dispose ??= context.tools.register(definition); };
    const unmount = () => { dispose?.(); dispose = undefined; };
    context.on('subagent/provider-added', (provider) => { if (provider.name === 'spawn')
        mount(); });
    context.on('subagent/provider-removed', (name) => { if (name === 'spawn')
        unmount(); });
    if (context.subagents.getProvider('spawn') !== undefined)
        mount();
}
//# sourceMappingURL=role-tool.js.map