...
export function parseBundledSkill(source) {
    ...
    if (!description)
        throw new Error(`Bundled skill "${name}" has no description.`);
    return { name, description, content: source.slice(match[0].length) };
}
export function dshSkillContent(name, content) {
...
function createBundledSkillProvider(providerName, skillRoot, content) {
    ...
    return {
        ...
        async get(candidate) {
            if (candidate.provider !== providerName || typeof candidate.path !== 'string')
                return undefined;
...
export function createOhStorySkillProvider(skillRoot = defaultBundledSkillRoot()) {
    return createBundledSkillProvider(STORY_PROVIDER_NAME, skillRoot, dshSkillContent);
}
export function dshDramaSkillContent(name, content) {
    const override = DSH_DRAMA_OVERRIDES[name];
    return `${DSH_DRAMA_BRIDGE}${override === undefined ? '' : `\n<skill-specific-dsh-override>${override}</skill-specific-dsh-override>`}\n\n${content}`;
}
export function createDramaSkillProvider(skillRoot = defaultDramaSkillRoot()) {
