...
function dshSkillContent(name, content) {
...
function defaultBundledSkillRoot() {
	...
	return basename(current) === "src" ? resolve(current, "../knowledge/oh-story/skills") : resolve(current, "oh-story/skills");
}
function defaultDramaSkillRoot() {
	const current = dirname(fileURLToPath(import.meta.url));
	return basename(current) === "src" ? resolve(current, "../knowledge/drama/skills") : resolve(current, "drama/skills");
}
function createBundledSkillProvider(providerName, skillRoot, content) {
	...
	return {
		...
		async get(candidate) {
			...
			if (relativePath === "" || relativePath.startsWith("..") || isAbsolute(relativePath)) throw new Error("Bundled skill locator escaped the packaged skill root.");
			const parsed = parseBundledSkill(await readFile(path, "utf8"));
...
function dshDramaSkillContent(name, content) {
...
//#endregion
export { apply, createDramaSkillProvider, createOhStorySkillProvider, types_default as default, defaultBundledSkillRoot, defaultDramaSkillRoot, inject, name, parseBundledSkill };
